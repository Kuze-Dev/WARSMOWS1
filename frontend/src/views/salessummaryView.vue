...
const fetchSalesSummary = async () => {
    ...
    try {
        ...
        if (salessummaries.value.length === 0 && currentPage.value > 1) {
            currentPage.value--;
            fetchSalesSummary(); // Fetch updated page data
        }
    } catch (error) {
...
const goToNextPage = () => {
    const totalPages = Math.ceil(totalTransactions.value / perPage.value);
...
<template>
    <main class="lg:w-[1200px] xl:w-[1310px] w-full h-screen  overflow-y-auto">
      ...
      <section class="lg:w-[95%] xl:w-[95%] w-[90%] border mt-2 lg:h-[565px] xl:h-[565px] h-[720px] rounded-md shadow-md">
         <div class="lg:flex mx-6 my-2 space-y-4 lg:space-y-0 lg:space-x-7 "> <!-- Added space-y-4 for small screens and space-x-7 for larger screens -->
                ...
                <div class="">
                    ...
                    <RouterLink to="/salessummary" class="flex">
                        ...
                        <span class="mt-3 font-bold">SALES SUMMARY</span>
                    </RouterLink>
                        ...
                        <div class="font-[sans-serif] overflow-x-auto border  h-[430px]">
                            <table class="w-full  bg-[#4E95C9]">
                                ...
                                <tbody class="bg-white whitespace-nowrap ">
                                 <tr class="border-b border-gray-300" v-if="salessummaries.length>0" v-for="(salessummary, index) in salessummaries"
                                 :key="salessummary.transaction_id">
                                 <td class="px-4 w-2 py-4 text-center  text-sm text-gray-800">
                                            {{ (currentPage - 1) * perPage + (index + 1) }}
                                        </td>
                                        <td class="px-4 w-2 py-4 text-sm  text-gray-800">
                                            <div class="text-[15px] font-bold">{{ salessummary.firstname }} {{
                                                salessummary.lastname }}</div>
                                            <div>{{ salessummary.sitio }}, {{ salessummary.barangay }},{{
                                                salessummary.city }}</div>
                                        </td>
                                        <td class="px-4 w-2 py-4 text-center  text-sm text-gray-800">
                                            <span
                                                :class="salessummary.selectedService === 'Delivery' ? 'bg-blue-400  rounded-full px-5 p-2' : 'bg-sky-500  rounded-full px-5 p-2'">
                                                {{ salessummary.selectedService }}
                                            </span>
                                        </td>
                                        <td class="px-4 w-2 py-4 text-center text-sm text-gray-800">
    <table class="font-[sans-serif] w-full">
        <tbody class="bg-white whitespace-nowrap">
            ...
            <!-- Totals Row -->
            <tr v-if="salessummary.transaction_items_concatenated.length" class="bg-gray-100 font-semibold">
            ...
            <tr v-if="salessummary.transaction_items_concatenated.length" class="bg-gray-100 font-semibold">
                <td class="px-4 py-4 text-center text-sm text-gray-800">
                    {{ salessummary.totalDue }}
                                        ...
                                        <td class="px-4 w-2 py-4 text-center text-sm text-gray-800">
                                            <span
                                                :class="salessummary.paymentStatus === 'Paid' ? 'bg-[#4E95C9]' : 'bg-red-500'"
                                                class=" rounded-full px-5 p-2">
                                                {{ salessummary.paymentStatus }}
                                            </span>
                                        </td>
                                        <td class="px-4 py-4 text-sm text-gray-800">
                                            {{ format(new Date(salessummary.transaction_date), 'MMMM dd, yyyy') }}
                                        </td>


                                 </tr>
                                  <!-- Display message if no sales summary are found -->
                                  <tr v-else>
                                        <td colspan="9" class="py-4 text-center text-gray-500 text-xl font-semibold">
                                            No sales summaries available.
                                        </td>
                                    </tr>
