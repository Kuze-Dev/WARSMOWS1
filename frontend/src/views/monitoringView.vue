...
const firstname = ref('');  // Initialize reactive variables asdasdasdsdasd sdfdasd
const lastname = ref('');
const sitio = ref('');
const barangay = ref('');
...
const transactionHistories = ref([]);  // Declare transactionHistories as a reactive reference
...
const totalTransactions = ref(0); // To track total customers for pagination
...
const goToNextPage = () => {
    const totalPages = Math.ceil(totalTransactions.value / perPage.value); // Calculate total pages
    if (currentPage.value < totalPages) {
        currentPage.value++;
        fetchTransactionHistory(); // Fetch users for the updated page
    }
};

const showOptionModal = ref(false);
const selectedHistoryId = ref(false);

const openOptionModal = (transactionHistoryId) => {
    if (showOptionModal.value && selectedHistoryId.value === transactionHistoryId) {
        closeOptionModal();
    } else {
        showOptionModal.value = true;
        selectedHistoryId.value = transactionHistoryId;
        // alert(selectedHistoryId.value)
        // alert(selectedHistoryId.value);
...
const openTransactionHistoryModal = (transactionHistoryId) => {
    showTransactionHistoryModal.value = true;
    // alert(transactionHistoryId);
    const transactionHistory = transactionHistories.value.find(
        transactionHistory => transactionHistory.transaction_id === transactionHistoryId
    );
    if (transactionHistory) {
        firstname.value = transactionHistory.firstname;
        lastname.value = transactionHistory.lastname;
        sitio.value = transactionHistory.sitio;
        barangay.value = transactionHistory.barangay;
    }
...
const closeTransactionHistoryModal = () => {
    ...
    currentPagePreviousHistory.value = 1; // Reset to page 1 when the modal is closed
}




const previoustransactionHistories = ref([]);

const fetchPreviousTransactionHistory = async () => {
    try {
        ...
        previoustransactionHistories.value = response.data.transactions.map(transaction => {
            return {
                ...transaction,
                // Parse transaction_items_concatenated as a JSON array
                transaction_items_history: JSON.parse(`[${transaction.transaction_items_history}]`)
            };
        });
...
const currentPagePreviousHistory = ref(1); // Current page
const itemsPerPage = ref(10); // Number of items per page

const paginatedTransactions = computed(() => {
    const start = (currentPagePreviousHistory.value - 1) * itemsPerPage.value;
...
const goToPreviousPageHistory = () => {
    if (currentPagePreviousHistory.value > 1) {
        currentPagePreviousHistory.value -= 1;
    }
    fetchPreviousTransactionHistory();

};

const goToNextPageHistory = () => {
    if (currentPagePreviousHistory.value < totalPages.value) {
        currentPagePreviousHistory.value += 1;
    }
    fetchPreviousTransactionHistory();
};
...
//         currentPagePreviousHistory.value = page;
//     }
//     fetchPreviousTransactionHistory();
// };
...
// Watch to update customer_id when a history ID is selected
watch(selectedHistoryId, () => {
    const selectedTransaction = previoustransactionHistories.value.find(
        (transaction) => transaction.transaction_id === selectedHistoryId.value
    );
    if (selectedTransaction) {
        selectedCustomerId.value = selectedTransaction.customer_id;
    } else {
...
const sortedTransactions = computed(() => {
    // Filter transactions based on matching conditions
    let filteredTransactions = previoustransactionHistories.value.filter((transaction) => {
        // Include transactions matching either selectedCustomerId or selectedHistoryId
        const matchesCustomer = selectedCustomerId.value
            ? transaction.customer_id === selectedCustomerId.value
            : true;
        const matchesHistory = selectedHistoryId.value
            ? transaction.transaction_id === selectedHistoryId.value
            : true;

        return matchesCustomer || matchesHistory;
    });
    ...
    return [...filteredTransactions].sort((a, b) => {
        ...
        return b.transaction_history_id - a.transaction_history_id;
    });
});



const deletePOS =async(transactionHistoryId)=>{
    // alert(transactionHistoryId);
    Swal.fire({
        title: 'Are you sure?',
        ...
        if (result.isConfirmed) {
            ...
            axios.delete(`/deletePOS/${transactionHistoryId}`).then((response) => {
...
<template>
    <main class="lg:w-[1200px] xl:w-[1310px] w-full h-screen overflow-y-auto">
        ...
        <div class="flex justify-center">
            <section
                class="lg:w-[95%] xl:w-[95%] w-[90%] border  lg:h-[667px] xl:h-[670px] h-800 rounded-md mt-7 shadow-md">
                <div class="lg:flex mx-3 my-2 space-y-4 lg:space-y-0 lg:space-x-7 ">
                    ...
                    <div class="">
                        <RouterLink to="/monitoring" class="flex">
                            ...
                            <span class="mt-3 font-bold">POS TRANSACTION</span>
                        </RouterLink>
                ...
                <div class="xl:flex lg:flex items-center px-3 py-2 mb-2 bg-gray-300">
                    ...
                    <div class="flex  mx-auto flex-col lg:w-[30%] xl:w-[30%] w-[97%] ">
                        ...
                        <div
                            ...
                            <input id="search-bar" type="search" v-model="searchQuery" @input="fetchTransactionHistory"
                                placeholder="Search ..."
                    ...
                    <div class="lg:flex xl:flex lg:space-x-5 xl:space-x-5 lg:ml-4 xl:ml-4 px-2 py-2">
                        ...
                        <div class="flex flex-col">
                            ...
                            <input id="input-1" type="text" v-model="searchQueryBarangay"
                                @input="fetchTransactionHistory" placeholder="All Barangay"
                        ...
                        <!-- Submit Button -->
                        <div class="flex flex-col">
                            <button @click="fetchTransactionHistory" id="filter-record-button" type="button"
                            ...
                            <table class=" w-full bg-[#4E95C9]">
                                <thead class="whitespace-nowrap">
                                    <tr>
                                        ...
                                        <th
                                            class="px-[15px] py-3 flex justify-center text-xs font-semibold  uppercase tracking-wider">
                                        ...
                                        <th
                                            class="px-4 py-3 flex justify-center text-xs font-semibold  uppercase tracking-wider ">
                                            <div class="flex ml-5 justify-center items-center">
                                                UNPAID
                                                <svg class="mx-2" width="21" height="20" viewBox="0 0 21 20" fill="none"
                                                    xmlns="http://www.w3.org/2000/svg">
                                ...
                                <tbody class="bg-white  whitespace-nowrap">
                                    <tr class="border-b border-gray-300" v-if="transactionHistories.length>0"
                                        v-for="(transactionHistory, index) in transactionHistories"
                                        :key="transactionHistory.transaction_id">
                                        <td class="px-4 w-2 py-4 text-center  text-sm text-gray-800">
                                            {{ (currentPage - 1) * perPage + (index + 1) }}
                                        </td>
                                        <td class="px-4 w-2 py-4 text-sm  text-gray-800">
                                            <div class="text-[15px] font-bold">{{ transactionHistory.firstname }} {{
                                                transactionHistory.lastname }}</div>
                                            <div>{{ transactionHistory.sitio }}, {{ transactionHistory.barangay }},{{
                                                transactionHistory.city }}</div>
                                        </td>
                                        <td class="px-4 w-2 py-4 text-center  text-sm text-gray-800">
                                            <span
                                                :class="transactionHistory.selectedService === 'Delivery' ? 'bg-blue-400  rounded-full px-5 p-2' : 'bg-sky-500  rounded-full px-5 p-2'">
                                                {{ transactionHistory.selectedService }}
                                            </span>
                                        </td>
                                        <td class="px-4 w-2 py-4 text-center text-sm text-gray-800">
        ...
        {{ item.quantity }}
            ...
            <tr v-if="transactionHistory.transaction_items_concatenated.length" class="bg-gray-100 font-semibold">
                ...
                <td class="px-4 py-4 text-center text-sm text-gray-800">
                    {{ transactionHistory.totalQuantity }}
                </td>
            </tr>
        </tbody>
    </table>
...
<td class="px-4 w-2 py-4 text-center text-sm text-gray-800">
    <table class="font-[sans-serif] w-full">
            ...
            <tr v-if="transactionHistory.transaction_items_concatenated.length" class="bg-gray-100 font-semibold">
                <td class="px-4 py-4 text-center text-sm text-gray-800">
                    {{ transactionHistory.totalDue }}
                                        ...
                                        <td class="px-4 w-2 py-4 text-center  text-sm text-gray-800">
                                            {{ transactionHistory.unpaid }}
                                        </td>
                                        <td class="px-4 w-2 py-4 text-center text-sm text-gray-800">
                                            <span
                                                :class="transactionHistory.paymentStatus === 'Paid' ? 'bg-[#4E95C9]' : 'bg-red-500'"
                                                class=" rounded-full px-5 p-2">
                                                {{ transactionHistory.paymentStatus }}
                                            </span>
                                        </td>
                                        <td class="px-4 w-2 py-4 text-sm text-gray-800">
                                            {{ format(new Date(transactionHistory.transaction_date), 'MMMM dd, yyyy') }}
                                        </td>
                                        <td class="px-4 w-2 py-4 text-sm  text-gray-800">

                                            <!--Start Of Option Item Icon -->
                                            <svg @click="openOptionModal(transactionHistory.transaction_id)"
                                            ...
                                            <section
                                                :class="showOptionModal && transactionHistory.transaction_id === selectedHistoryId ? 'modal-content relative mx-2  font-[sans-serif] right-[75px]' : 'hidden'"
                                                v-show="transactionHistory.transaction_id === selectedHistoryId">
                                                <div v-for="(transactionHistory, index) in transactionHistories"
                                                    :key="transactionHistory.transaction_id"
                                                    v-show="transactionHistory.transaction_id === selectedHistoryId"
                                                    ...
                                                    <div class="flex flex-wrap">
                                                        ...
                                                        <div class="mx-3 flex justify-center items-center mt-2 pb-1">
                                                            ...
                                                            <span
                                                                @click="openTransactionHistoryModal(transactionHistory.transaction_id)"
                                                        ...
                                                        <div class="mx-4 flex justify-center items-center mt-1 ">
                                                            ...
                                                            <span  @click="deletePOS(transactionHistory.customer_id)"
                                     ...
                                     <!-- Display message if no pos transaction are found -->
                                     <tr v-else>
                                        <td colspan="10" class="py-4 text-center text-gray-500 text-xl font-semibold">
                                            No POS transaction available.
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
        ...
        <section v-if="showTransactionHistoryModal" @click.self="closeTransactionHistoryModal"
            ...
            <div class="lg:w-[82%] xl:w-[83%]  w-full  border bg-white py-2 relative rounded-lg pb-4">
                <div class="flex pb-3 border-b  mx-3 border-black">
                    <div class=" flex flex-1">
                        ...
                        <h3 class="text-gray-800 mt-1  font-bold">TRANSACTION HISTORY</h3>
                    </div>

                    <svg @click="closeTransactionHistoryModal" xmlns="http://www.w3.org/2000/svg"
                ...
                <div class="mx-3 mt-3">
                    <div class="flex justify-start ">
                        <svg class="mr-2 mt-1 " width="27" height="23" viewBox="0 0 27 26" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M1.13074 21.2931C1.13074 19.7902 1.76288 18.3489 2.8881 17.2862C4.01331 16.2235 5.53944 15.6265 7.13074 15.6265H19.1307C20.722 15.6265 22.2482 16.2235 23.3734 17.2862C24.4986 18.3489 25.1307 19.7902 25.1307 21.2931C25.1307 22.0446 24.8147 22.7652 24.2521 23.2966C23.6894 23.828 22.9264 24.1265 22.1307 24.1265H4.13074C3.33509 24.1265 2.57203 23.828 2.00942 23.2966C1.44681 22.7652 1.13074 22.0446 1.13074 21.2931Z"
                                stroke="#555555" stroke-width="2" stroke-linejoin="round" />
                            <path
                                d="M13.1307 9.95996C15.616 9.95996 17.6307 8.05717 17.6307 5.70996C17.6307 3.36275 15.616 1.45996 13.1307 1.45996C10.6455 1.45996 8.63074 3.36275 8.63074 5.70996C8.63074 8.05717 10.6455 9.95996 13.1307 9.95996Z"
                                stroke="#555555" stroke-width="2" />
                        </svg>

                        <div>
                            <h4 class=" text-[22px] text-gray-800 uppercase font-bold">{{ firstname }} {{ lastname }}
                            </h4>
                        </div>
                    </div>
                    <div>
                        <h4 class=" text-[15px] mx-9 text-gray-800 uppercase ">{{ sitio }}, {{ barangay }}</h4>
                    </div>
                </div>
                ...
                <section class="mx-3">
                    <div class="w-full  py-3">
                        <div class="font-[sans-serif] overflow-x-auto h-[610px]  border ">
                            <table class=" w-[50px] bg-[#4E95C9]">
                                <thead class="whitespace-nowrap">
                                    <tr>
                                        <th class="px-5 py-4 text-left text-xs font-semibold  uppercase tracking-wider">
                                            #
                                ...
                                <tbody class="bg-white whitespace-nowrap">
                                    ...
                                    <tr v-for="(transaction, index) in paginatedTransactions"
                                        :key="transaction.transaction_history_id">
                                        <td class="px-4 w-2 py-4 text-sm text-gray-800">
                                            {{ (currentPagePreviousHistory - 1) * itemsPerPage + index + 1 }}
                                        </td>

                                        <td class="px-4 w-2 py-4 text-sm text-gray-800">

                                            <span
                                                :class="transaction.selectedService === 'Delivery' ? 'bg-blue-400  rounded-full px-5 p-2' : 'bg-sky-500  rounded-full px-5 p-2'">
                                                {{ transaction.selectedService }}
                                            </span>
    ...
    <table class="font-[sans-serif] w-full">
        <tbody class="bg-white whitespace-nowrap">
            <!-- Loop through transaction items -->
            <tr v-for="(history, index) in transaction.transaction_items_history" :key="history.item_id" class="border-b border-gray-300">
                <td class="px-4 w-2 py-4 text-center text-sm text-gray-800">
                    <div class="text-[0.7rem] text-gray-800 ">
                                                <span class="mt-2 block">{{ history.title }}</span>
                                            </div>
        </td>
                <td class="px-4 py-4 text-center text-sm text-gray-800">
                    {{ history.quantity }}
            ...
            <!-- Totals Row -->
            <tr v-if="transaction.transaction_items_history.length" class="bg-gray-100 font-semibold">
                <td class="px-4 w-2 py-4 text-sm text-gray-800">Total:</td>
            ...
            <tr v-if="transaction.transaction_items_history.length" class="bg-gray-100 font-semibold">
                <td class="px-4 py-4 text-center text-sm text-gray-800">
                    {{ transaction.totalDue }}
                                        ...
                                        <td class="px-4 w-2 py-4 text-sm text-gray-800">
                                            <span
                                                :class="transaction.paymentStatus === 'Paid' ? 'bg-[#4E95C9]' : 'bg-red-500'"
                                                class=" rounded-full px-5 p-2">
                    ...
                    <div class="flex justify-center">
                        ...
                        <svg @click="goToPreviousPageHistory" class="bg-gray-200 border-l rounded-l-full " width="40"
                            height="30" viewBox="0 0 24 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" clip-rule="evenodd"
                                d="M11.707 7.05451C11.8945 7.24203 11.9998 7.49634 11.9998 7.76151C11.9998 8.02667 11.8945 8.28098 11.707 8.4685L7.414 12.7615L11.707 17.0545C11.8892 17.2431 11.99 17.4957 11.9877 17.7579C11.9854 18.0201 11.8802 18.2709 11.6948 18.4563C11.5094 18.6417 11.2586 18.7469 10.9964 18.7492C10.7342 18.7515 10.4816 18.6507 10.293 18.4685L5.293 13.4685C5.10553 13.281 5.00021 13.0267 5.00021 12.7615C5.00021 12.4963 5.10553 12.242 5.293 12.0545L10.293 7.05451C10.4805 6.86703 10.7348 6.76172 11 6.76172C11.2652 6.76172 11.5195 6.86703 11.707 7.05451ZM17.707 7.05451C17.8945 7.24203 17.9998 7.49634 17.9998 7.76151C17.9998 8.02667 17.8945 8.28098 17.707 8.4685L13.414 12.7615L17.707 17.0545C17.8892 17.2431 17.99 17.4957 17.9877 17.7579C17.9854 18.0201 17.8802 18.2709 17.6948 18.4563C17.5094 18.6417 17.2586 18.7469 16.9964 18.7492C16.7342 18.7515 16.4816 18.6507 16.293 18.4685L11.293 13.4685C11.1055 13.281 11.0002 13.0267 11.0002 12.7615C11.0002 12.4963 11.1055 12.242 11.293 12.0545L16.293 7.05451C16.4805 6.86703 16.7348 6.76172 17 6.76172C17.2652 6.76172 17.5195 6.86703 17.707 7.05451Z"
                                fill="#555555" />
                        ...
                        <span class="bg-[#4E95C9] px-3 pt-1 hover:text-white ">
                            {{ currentPagePreviousHistory }}
                        </span>

                        <!-- Next Page Button -->
                        <svg @click="goToNextPageHistory" class="bg-gray-200 border-r rounded-r-full" width="40"
                            height="30" viewBox="0 0 26 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" clip-rule="evenodd"
                                d="M13.2024 18.473C13.0179 18.2825 12.9166 18.0265 12.9208 17.7614C12.9251 17.4963 13.0344 17.2437 13.2248 17.0591L17.5854 12.8348L13.361 8.47422C13.1818 8.28275 13.0851 8.02858 13.0915 7.76645C13.0979 7.50432 13.2071 7.25521 13.3954 7.07277C13.5837 6.89032 13.8362 6.78914 14.0984 6.79102C14.3606 6.7929 14.6115 6.89769 14.7972 7.08282L19.7173 12.1615C19.9018 12.352 20.003 12.6079 19.9988 12.873C19.9946 13.1382 19.8853 13.3908 19.6949 13.5753L14.6162 18.4954C14.4257 18.6799 14.1698 18.7811 13.9047 18.7769C13.6395 18.7727 13.3869 18.6634 13.2024 18.473ZM7.20314 18.3778C7.01867 18.1873 6.9174 17.9314 6.9216 17.6662C6.92581 17.4011 7.03514 17.1485 7.22557 16.964L11.5861 12.7396L7.36173 8.37906C7.18259 8.1876 7.08581 7.93343 7.09225 7.6713C7.09868 7.40917 7.20782 7.16006 7.39614 6.97761C7.58447 6.79517 7.83692 6.69399 8.09912 6.69587C8.36132 6.69775 8.61229 6.80254 8.79798 6.98766L13.7181 12.0663C13.9025 12.2568 14.0038 12.5128 13.9996 12.7779C13.9954 13.043 13.8861 13.2956 13.6956 13.4802L8.61696 18.4002C8.42649 18.5847 8.17054 18.686 7.90541 18.6818C7.64028 18.6776 7.38767 18.5682 7.20314 18.3778Z"
                                fill="#555555" />
