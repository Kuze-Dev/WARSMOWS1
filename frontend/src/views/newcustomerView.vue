...
const fetchNewCustomer = async () => {
    try {
        const response = await axios.get(`/customer?page=${currentPage.value}&limit=${perPage.value}`);
        newcustomers.value = response.data.customers;
        // console.log('New Customer', newcustomers.value);
        ...
        if (newcustomers.value.length === 0 && currentPage.value > 1) {
            // If the page is empty, go to the previous page
            currentPage.value--;
            fetchNewCustomer(); // Fetch users for the updated page
        }
    } catch (error) {
...
const perPage = ref(10); // Number of customers per page
...
<template>
    <main class="lg:w-[1200px] xl:w-[1310px] w-full h-screen overflow-y-auto">
        ...
        <div class="flex justify-center">
            ...
            <section
                class="lg:w-[95%] xl:w-[95%] w-[90%] border mt-2 lg:h-[728px] xl:h-[728px] h-[885px] rounded-md shadow-md">
                <div class="lg:flex mx-6 my-2 space-y-4 lg:space-y-0 lg:space-x-7 ">
                    ...
                    <div class="mt-2">
                        <RouterLink to="/newcustomer" class="flex">
                            ...
                            <span class="mt-3 font-bold">NEW CUSTOMER</span>
                        </RouterLink>
                ...
                <section class="mt-2">
                    <div class="w-full  h-[270px]">
                        <div class="font-[sans-serif] overflow-x-auto border h-[593px]">
                            <table class=" w-full bg-[#4E95C9]">
                                ...
                                <tbody class="bg-white  whitespace-nowrap">
                                    <tr class="border-b" v-if="newcustomers.length > 0"
                                        v-for="(newcustomer, index) in newcustomers" :key="newcustomer.customer_id">
                                        ...
                                        <td class="px-4 w-2 py-4 text-center text-sm uppercase text-gray-800">
                                            {{ newcustomer.sitio }}, {{ newcustomer.barangay }}, {{ newcustomer.city }},
                                            {{ newcustomer.province }}
                                        </td>
                                        <td class="px-4 w-2 py-4 text-center text-sm uppercase text-gray-800">
                                            {{ newcustomer.user_firstname }} {{ newcustomer.user_lastname }}
                                        </td>
                                        <td class="px-4 w-2 py-4 text-center text-sm uppercase text-gray-800">
                                            {{ format(new Date(newcustomer.date_customerAdded), 'MMMM dd, yyyy') }}
                                        </td>

                                    </tr>
                                    <!-- Display message if no new customers are found -->
                                    <tr v-else>
